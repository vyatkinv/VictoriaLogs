...
│    │    │    │    ├─ tls.X509KeyPair(cert, key)  (валидация)      │    Vault HTTP API
│    │    │    │    └─ writePEMFiles(cert, key)  атомарно на диск   │
│    │    │    └─ go backgroundRenewer()                             │
│    │    │         └─ sleep until expiry − (lifetime/3)             │
│    │    │              └─ renew() → перезапись cert.pem/key.pem ───┼──► POST /v1/pki/issue/role
│    │    │                                                          │
│    │    ├─ flag.Set("tls", "true")                                 │
...
**Почему без правок vendor:** штатный `newGetCertificateFunc` в `netutil` уже перечитывает cert/key-файлы с диска примерно раз в секунду. Провайдер Vault просто держит эти файлы актуальными, а `initVaultTLS` направляет на них флаги `-tlsCertFile`/`-tlsKeyFile`. Никакие файлы в `vendor/` не модифицируются — `make vendor-update` остаётся безопасным.

**Поток данных при каждом TLS-рукопожатии:**

1. Клиент инициирует TLS → Go runtime вызывает `cfg.GetCertificate(hello)` (стандартный vendored-колбэк).
2. Колбэк раз в секунду перечитывает `cert.pem`/`key.pem` через `LoadX509KeyPair`, иначе отдаёт кэш.
3. Провайдер Vault в фоне переписывает эти файлы до истечения сертификата, поэтому обновление подхватывается прозрачно.
...
Ключ записывается раньше сертификата: если vendored-reader между двумя `rename` подхватит новый cert, соответствующий ему key уже на диске, и `LoadX509KeyPair` не упадёт на несовпадении пары. Этот же file-based путь покрывает оба TLS-слушателя (HTTP и syslog TCP), поскольку оба вызывают `netutil.GetServerTLSConfig`.
...
```
-tls.vaultAddr задан?
    ДА  → Vault PKI управляет -tlsCertFile/-tlsKeyFile сам;
