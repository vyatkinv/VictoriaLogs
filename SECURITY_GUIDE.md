...
- **HTTP** — `tls.Config` собирается внутри vendored `httpserver.Serve`, инъекции нет. Но штатный `newGetCertificateFunc` уже перечитывает cert/key-файлы примерно раз в секунду, поэтому провайдер держит PEM-файлы актуальными, а `initVaultTLS` направляет на них `-tlsCertFile`/`-tlsKeyFile`. Файлы пишутся в `/dev/shm` (tmpfs, в RAM) с правами `0600` — приватный ключ не касается постоянного диска; фолбэк на `os.TempDir()`, если tmpfs недоступен.
