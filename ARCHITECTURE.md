...
Позволяет получать TLS-сертификаты напрямую из HashiCorp Vault PKI Secrets Engine вместо файлов на диске, с автоматической горячей заменой без перезапуска. Один и тот же провайдер обслуживает три роли: **входящий** TLS HTTP- и syslog-слушателей (серверный сертификат), а также **исходящий** TLS к `-storageNode` и `-remoteWrite.url` (клиентский сертификат и/или доверие CA). Пакет флагов `lib/vaulttls/vaultflags` общий для `victoria-logs` и `vlagent`.
...
| `lib/vaulttls/auth.go` | Методы аутентификации (`token`/`approle`/`kubernetes`), кэш токена, unwrap response-wrapped secret_id |
| `lib/vaulttls/client.go` | Исходящий TLS: `GetClientCertificate`, загрузка PKI CA, `NewRoundTripper` для клиентов |
| `lib/vaulttls/vaultflags/` | CLI-флаги `-tls.vault*`; `Init()`/`Stop()`, общие для victoria-logs и vlagent |
...
```
main()
 ├─ vaultflags.Init()                       ← ДО vlstorage.Init() и vlinsert.Init()
 │    ├─ vaulttls.NewProvider(cfg)          ← lib/vaulttls/vaulttls.go
 │    │    ├─ renew() → fetchCert()                 Vault HTTP API
 │    │    │    ├─ tokenSource.get()  ──────────► POST /v1/sys/wrapping/unwrap   (только approle+wrapped)
 │    │    │    │                       ────────► POST /v1/auth/{mount}/login    (approle | kubernetes)
 │    │    │    ├─ issueCert(token)  ───────────► POST /v1/{pki}/issue/{role}
 │    │    │    └─ tls.X509KeyPair → p.cert
 │    │    ├─ refreshCA()  ─────────────────────► GET /v1/{pki}/cert/ca_chain    (только -tls.vaultTrustPKICA, без токена)
 │    │    └─ go backgroundRenewer()
 │    ├─ vaulttls.Register(p)
 │    └─ flag.Set("tls", "true")            (включает схему https, но не файлы; только если -tls не задан явно)
 ├─ vlstorage.Init()
 │    └─ netinsert/netselect: vaulttls.NewRoundTripper(ac, tr)   ← клиент к -storageNode
 ├─ vlselect.Init()
 ├─ vlinsert.Init()
...
Порядок важен: `vaultflags.Init()` вызывается **до** `vlstorage.Init()` (создаёт клиентов к `-storageNode`) и **до** `vlinsert.Init()` (поднимает syslog-слушатель) — оба берут сертификат из уже зарегистрированного провайдера. Для HTTP порядок безразличен: `GetTLSConfig` вызывается лениво внутри `httpserver.Serve`. У `vlagent` схема та же, только вместо `-storageNode` клиент строится в `remotewrite` для `-remoteWrite.url`.
...
type Provider struct {
    cfg      Config
    client   *http.Client
    auth     *tokenSource      // логин в Vault, кэш токена — lib/vaulttls/auth.go

    mu       sync.Mutex
    cert     *tls.Certificate  // отдаётся через GetCertificate обоим слушателям
    serial   string            // serial текущего сертификата — для pki/revoke
    expiry   time.Time
...
`Stop()` останавливает фоновую горутину, отзывает сертификат (при `-tls.vaultRevokeOnShutdown`) и всегда отзывает свой Vault-токен через `auth/token/revoke-self` — чтобы утёкший из памяти токен нельзя было переиспользовать после выхода. Оба отзыва best-effort: ошибка только логируется.

**Сетевой ввод-вывод не выполняется под `p.mu`.** `renew()` разделён на `fetchCert()` (логин + выпуск, без лока) и присвоение полей под мьютексом. Иначе рукопожатия, читающие `p.cert`, блокировались бы на время запроса к Vault — до 30 с таймаута, теперь ещё и с логином внутри.

### Аутентификация

`tokenSource` (`lib/vaulttls/auth.go`) реализует три метода:

| Метод | Запрос | Что перечитывается |
|-------|--------|--------------------|
| `token` | нет | `-tls.vaultTokenFile` при каждом обращении |
| `approle` | `POST /v1/auth/{mount}/login` с `role_id`+`secret_id` | файл `secret_id`; wrapped-вариант распаковывается один раз за процесс |
| `kubernetes` | `POST /v1/auth/{mount}/login` с `role`+`jwt` | JWT при каждом логине — это и покрывает ротацию projected-токена |

Токен кэшируется и меняется, когда израсходовано 2/3 лизы (`lease_duration == 0` → токен считается вечным). Отдельной горутины продления нет: токен нужен лишь в момент выпуска сертификата, а повторный логин надёжнее `renew-self`, упирающегося в `token_max_ttl`. Если Vault ответил 401/403 на выпуск, выполняется один принудительный релогин и повтор — для `token` это бессмысленно и пропускается.
...
| Токен истёк по лизе | Прозрачный релогин перед выпуском (`approle`/`kubernetes`) |
| Токен отозван досрочно | HTTP 401/403 на выпуске → принудительный релогин и один повтор; для `token` — ошибка и retry через 10 с |
| Wrapping-токен уже использован | `Fatalf` на старте: одноразовый токен мог быть перехвачен — нужен новый wrapped secret_id |
| Заданы и `-tls.vaultAddr`, и `-tlsCertFile` | `logger.Fatalf` — конфликт конфигурации |
| Флаг чужого auth-метода | `logger.Fatalf` с указанием, какой `-tls.vaultAuthMethod` нужен |
...
### Исходящий TLS (клиент)

`lib/vaulttls/client.go` подключает тот же провайдер к исходящим соединениям через `vaulttls.NewRoundTripper(ac, tr)` — drop-in-замену `ac.NewRoundTripper(tr)` в трёх местах: `app/vlstorage/netinsert`, `app/vlstorage/netselect` (к `-storageNode`) и `app/vlagent/remotewrite` (к `-remoteWrite.url`). Если провайдер не зарегистрирован или оба флага (`-tls.vaultClientAuth`, `-tls.vaultTrustPKICA`) выключены, возвращается неизменённый `promauth`-round-tripper, поэтому call-site'ы не содержат условий.

| Флаг | Механизм | Требование |
|------|----------|-----------|
| `-tls.vaultClientAuth` | `tls.Config.GetClientCertificate` отдаёт текущий `p.cert` под мьютексом | PKI-роль с `client_flag=true`; несовместим с `*.tlsCertFile` |
| `-tls.vaultTrustPKICA` | CA mount'а добавляется в `tls.Config.RootCAs` поверх системного пула и `*.tlsCAFile` | — |

Три свойства реализации:

- **CA без токена и без файла.** `refreshCA()` читает `GET {pki}/cert/ca_chain` (откат на `{pki}/cert/ca`) — путь неаутентифицированный, поэтому запрос идёт без `X-Vault-Token`. Результат хранится в `atomic.Pointer[caBundle]` и перечитывается после каждого успешного продления, так что ротация корневого CA подхватывается без рестарта. Дедупликация по `bytes.Equal` — лог пишется один раз при изменении.
- **Настроенный CA не теряется.** Если у соединения задан `*.tlsCAFile`, `vaultRoundTripper` клонирует `tls.Config` и `AppendCertsFromPEM` добавляет Vault CA к нему, а не заменяет.
- **Кэш транспорта по идентичности.** `vaultRoundTripper` пересобирает `http.Transport` только когда меняется указатель `tls.Config` или `caBundle`; ротация клиентского сертификата идёт через колбэк и пересборки не требует.

`vlagent` использует обе стороны сразу: `httpserver` отдаёт входящий TLS из провайдера, а `remotewrite` — исходящий mTLS к victoria-logs, где и клиентский сертификат, и CA приходят из Vault (ни одного PEM-файла).

**Серверная проверка клиентского сертификата** на стороне storage-узла (`ClientCAs`) в объём не входит: форк `lib/httpserver` её не пробрасывает. Storage-узел проверяет клиента файловым `-tlsCAFile`; Vault закрывает только клиентскую сторону соединения.
