...
### Ключевое решение: никаких правок vendor

Vendored `netutil.GetServerTLSConfig` уже раздаёт файловые сертификаты через `newGetCertificateFunc`, который **перечитывает cert/key с диска примерно раз в секунду**. Интеграция использует именно этот механизм: провайдер Vault пишет выпущенный сертификат в PEM-файлы во временном каталоге и указывает на них `-tlsCertFile`/`-tlsKeyFile` через `flag.Set(...)`. При обновлении файлы переписываются — штатный file-reader подхватывает их сам.

Так интеграция не трогает ни одного файла в `vendor/`, что сохраняет возможность `make vendor-update`.
...
main()
 ...
 │    ├─ (проверка: -tlsCertFile/-tlsKeyFile не заданы вручную)
 │    ├─ vaulttls.NewProvider(cfg)          ← lib/vaulttls/vaulttls.go
 │    │    ├─ os.MkdirTemp() → cert.pem, key.pem
 ...
 │    ├─ flag.Set("tls", "true")
 │    ├─ flag.Set("tlsCertFile", provider.CertFile())
 │    └─ flag.Set("tlsKeyFile",  provider.KeyFile())
 └─ go httpserver.Serve()
       └─ netutil.GetServerTLSConfig(cert.pem, key.pem, ...)   ← без изменений
            └─ cfg.GetCertificate = newGetCertificateFunc(...)  перечитывает раз в секунду
```

Один и тот же file-based путь покрывает оба слушателя — HTTP (`httpserver`) и syslog TCP (`syslog.go`) — так как оба вызывают `netutil.GetServerTLSConfig` с этими же флагами.

### Атомарность записи файлов

`writePEMFiles` пишет во временный файл и делает `os.Rename` (атомарный на одной ФС), чтобы читатель никогда не увидел частично записанный файл. Ключ пишется **раньше** сертификата: если reader между двумя переименованиями подхватит новый cert, соответствующий ему новый key уже на месте, и `LoadX509KeyPair` не упадёт на несовпадении пары.
...
type Provider struct {
    ...
    dir      string        // временный каталог с PEM-файлами
