...
| Файл | Роль |
|------|------|
| `lib/vaulttls/vaulttls.go` | Vault PKI-клиент: выпуск, запись PEM-файлов, фоновое обновление |
| `app/victoria-logs/vault_tls.go` | CLI-флаги; `initVaultTLS()` до старта HTTP-сервера |

### Ключевое решение: никаких правок vendor + два способа выдачи

У VictoriaLogs два вида TLS-слушателей, и `tls.Config` для них строится в разных местах. Vault-провайдер отдаёт сертификат обоими способами, оба — без единой правки в `vendor/`:

| Слушатель | Где строится `tls.Config` | Способ выдачи cert |
|-----------|---------------------------|--------------------|
| HTTP (`:9428`) | внутри vendored `httpserver.Serve` | **файлы** — `-tlsCertFile`/`-tlsKeyFile` |
| syslog TCP | в нашем `app/vlinsert/syslog/syslog.go` | **память** — `GetCertificate`-колбэк |
...
main()
 ...
 │    │    │    ├─ tls.X509KeyPair → p.cert (в память)   Vault HTTP API
 │    │    │    └─ writePEMFiles(cert, key)  атомарно: write .tmp → rename
 │    │    └─ go backgroundRenewer()
 │    ├─ vaulttls.Register(p)               (для syslog — in-memory)
 │    ├─ flag.Set("tls", "true")
 │    ├─ flag.Set("tlsCertFile", provider.CertFile())   (для HTTP — файлы)
 │    └─ flag.Set("tlsKeyFile",  provider.KeyFile())
 ├─ vlinsert.Init()
 │    └─ syslog.runTCPListener()
 │         └─ vaulttls.ServerTLSConfig(...) → cfg.GetCertificate = p.GetCertificate
 └─ go httpserver.Serve()
       └─ netutil.GetServerTLSConfig(cert.pem, key.pem, ...)   ← vendored, без изменений
            └─ cfg.GetCertificate = newGetCertificateFunc(...)  перечитывает раз в секунду
```

Порядок важен: `initVaultTLS()` вызывается **до** `vlinsert.Init()`, потому что syslog-слушатель поднимается внутри `vlinsert.Init()` и на этот момент провайдер уже должен быть зарегистрирован.

### Атомарность записи файлов (HTTP)

`writePEMFiles` пишет во временный файл и делает `os.Rename` (атомарный на одной ФС), чтобы читатель никогда не увидел частично записанный файл. Ключ пишется **раньше** сертификата: если reader между двумя переименованиями подхватит новый cert, соответствующий ему новый key уже на месте, и `LoadX509KeyPair` не упадёт на несовпадении пары. Для syslog этой проблемы нет — там подмена одного указателя `p.cert` под мьютексом.
...
type Provider struct {
    ...
    dir      string        // временный каталог с PEM-файлами (HTTP)
    certPath string        // → -tlsCertFile
    keyPath  string        // → -tlsKeyFile

    mu       sync.Mutex
    cert     *tls.Certificate  // in-memory копия, отдаётся через GetCertificate (syslog)
    expiry   time.Time
...
`CertFile()`/`KeyFile()` отдают пути для флагов. `Stop()` останавливает горутину и удаляет временный каталог.
...
| Vault недоступен при обновлении | Warn-лог, retry через 10 с; текущие PEM-файлы остаются валидными до истечения cert |
| Cert истёк, Vault недоступен | Файлы не обновляются; `LoadX509KeyPair` отдаёт истёкший cert — клиент отвергает рукопожатие |
...
```
-tls.vaultAddr задан
    ДА  → HTTP:   Vault PKI через файлы; -tlsCertFile/-tlsKeyFile нельзя задавать вручную
